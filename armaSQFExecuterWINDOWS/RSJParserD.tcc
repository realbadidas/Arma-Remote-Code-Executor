...
enum BETRSJREsourceType { RSJ_UNINITIATED, RSJ_UNKNOWN, RSJ_OBJECT, RSJ_ARRAY, RSJ_LEAF };

// ============================================================
// Direct string manipulation functions

inline 
std::string to_string (BETRSJREsourceType rt) {
    switch (rt) {
...
class RSJparsedData;
class BETRSJREsource;

// Objet and array typedefs
typedef std::unordered_map <std::string,BETRSJREsource>    RSJobject;
typedef std::vector <BETRSJREsource>                       RSJarray;

// ------------------------------------
// Main classes

class BETRSJREsource {
/* Use: BETRSJREsource("RSJ_string_data").as<RSJobject>()["keyName"].as<RSJarray>()[2].as<int>()
        BETRSJREsource("RSJ_string_data")["keyName"][2].as<int>()  */
private:
    // main data
...
public:
    // constructor
    BETRSJREsource () : _exists (false), parsed_data_p (NULL) { } // no data field.
    
    BETRSJREsource (std::string str) : data (str), _exists (true), parsed_data_p (NULL) { }
    BETRSJREsource (const char* str) : BETRSJREsource(std::string(str)) { }
    
    // other convertion
    template <class dataType>
    BETRSJREsource (dataType d) : BETRSJREsource(std::to_string(d)) { }
    
    // read from file and stream
    BETRSJREsource (std::istream& is) : _exists (true), parsed_data_p (NULL) {
        data = std::string ( (std::istreambuf_iterator<char>(is)), (std::istreambuf_iterator<char>()) );
    }
    BETRSJREsource (std::ifstream& ifs) : _exists (true), parsed_data_p (NULL) {
        std::istream& is = ifs;
        data = std::string ( (std::istreambuf_iterator<char>(is)), (std::istreambuf_iterator<char>()) );
    }
    ...
    // deep copy
    BETRSJREsource (const BETRSJREsource& r);
    BETRSJREsource& operator= (const BETRSJREsource& r);
    
    // ------------------------------------
    // parsers (old)
    BETRSJREsourceType parse (bool force=false);
    ...
    // opertor[]
    BETRSJREsource& operator[] (std::string key); // object
    BETRSJREsource& operator[] (int indx); // array
...
public:
    ...
    BETRSJREsourceType type;
    RSJparsedData() : type(RSJ_UNKNOWN) {}
    
    // parser (single-level)
    void parse (const std::string& data, BETRSJREsourceType typ = RSJ_UNKNOWN) {
            ...
            if (content.length() != data.length()) { // a valid object
                ...
                for (int a=0; a<nvPairs.size(); ++a) {
                    ...
                    object.insert (make_pair( 
                                        ...
                                        BETRSJREsource (strtrim (nvPairs[a].substr (assignmentPos+1) ) )
                               ) );
                }
                if (object.size() > 0) {
                    type = RSJ_OBJECT;
        ...
        if (typ==RSJ_ARRAY || typ==RSJ_UNKNOWN) {
            ...
            if (content.length() != data.length()) { // a valid array
                std::vector<std::string> nvPairs = split_RSJ_array (content);
                for (int a=0; a<nvPairs.size(); ++a) 
                    array.push_back (BETRSJREsource (strtrim (nvPairs[a]) ) );
                if (array.size() > 0) {
                    type = RSJ_ARRAY;
                    return;
                }
...
// ------------------------------------------------------------
// BETRSJREsource member functions

inline 
BETRSJREsource::~BETRSJREsource (){
    if (parsed_data_p) delete parsed_data_p;
}

inline 
BETRSJREsource::BETRSJREsource (const BETRSJREsource& r) {
    data=r.data;
    _exists = r._exists;
...
inline 
BETRSJREsource& BETRSJREsource::operator= (const BETRSJREsource& r) {
    data=r.data;
    _exists = r._exists;
    if(r.parsed_data_p) parsed_data_p = new RSJparsedData(*(r.parsed_data_p));
    else parsed_data_p = NULL;
...
inline 
BETRSJREsourceType BETRSJREsource::type (void) {
...
inline 
std::string BETRSJREsource::as_str (bool print_comments, bool update_data) {
    if (exists()) {
        std::string ret;
        parse(); // parse if not parsed
...
inline 
BETRSJREsourceType BETRSJREsource::parse (bool force) {
...
inline 
void BETRSJREsource::parse_full (bool force, int max_depth, int* parse_count_for_verbose_p) { // recursive parsing (slow)
...
inline 
void BETRSJREsource::fast_parse (std::string* str_p, bool copy_string, int max_depth, int* parse_start_str_pos) {
...
inline 
RSJobject& BETRSJREsource::as_object (bool force) {
    if (!parsed_data_p)  parsed_data_p = new RSJparsedData;
...
inline 
BETRSJREsource& BETRSJREsource::operator[] (std::string key) { // returns reference
...
inline 
RSJarray& BETRSJREsource::as_array (bool force) {
    if (!parsed_data_p)  parsed_data_p = new RSJparsedData;
...
inline 
BETRSJREsource& BETRSJREsource::operator[] (int indx) { // returns reference
    as_array();
...
vectorType BETRSJREsource::as_vector (const vectorType& def) { // returns copy -- for being consistent with other 'as' specializations
    if (!exists()) return (def);
    vectorType ret;
    as_array();
    for (auto it=parsed_data_p->array.begin(); it!=parsed_data_p->array.end(); ++it)
...
mapType BETRSJREsource::as_map (const mapType& def) { // returns copy -- for being consistent with other 'as' specializations
    if (!exists()) return (def);
    mapType ret;
    as_object();
    for (auto it=parsed_data_p->object.begin(); it!=parsed_data_p->object.end(); ++it)
...
// RSJobject
template <> inline 
RSJobject BETRSJREsource::as<RSJobject> (const RSJobject& def) { // returns copy -- for being consistent with other 'as' specializations
    if (!exists()) return (def);
    return (as_object());
}
...
// RSJarray
template <> inline 
RSJarray  BETRSJREsource::as<RSJarray> (const RSJarray& def) { // returns copy -- for being consistent with other 'as' specializations
    if (!exists()) return (def);
    return (as_array());
}
...
// String
template <> inline 
std::string  BETRSJREsource::as<std::string> (const std::string& def) {
    if (!exists()) return (def);
...
// integer
template <> inline 
int  BETRSJREsource::as<int> (const int& def) {
    if (!exists()) return (def);
...
// double
template <> inline 
double  BETRSJREsource::as<double> (const double& def) {
    if (!exists()) return (def);
...
// bool
template <> inline 
bool  BETRSJREsource::as<bool> (const bool& def) {
    if (!exists()) return (def);
...
bool  BETRSJREsource::as< std::vector<dataType> > (const std::vector<dataType>& def) {
    return as_vector<dataType> (def);
}

template <> template <dataType> inline 
std::unordered_map<std::string,dataType>  BETRSJREsource::as< std::unordered_map<std::string,dataType> > 
